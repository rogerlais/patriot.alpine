...
#****removed by older bash use -> LOG_LEVELS=([0]="emerg" [1]="alert" [2]="crit" [3]="err" [4]="warning" [5]="notice" [6]="info" [7]="debug")
...
function slog() {
	...
	if [[ ${APP_VERBOSE_LEVEL} -ge ${LEVEL} ]]; then
		if [ -t 0 ]; then
			#echo "[${LOG_LEVELS[$LEVEL]}]" "$@" | tee -a "$APP_LOG_FILE"  #todo:refactor to old bash, need validation
			echo "['echo $LOG_LEVELS | cut -d ' ' -f$((LEVEL + 1))']" "$@" | tee -a "$APP_LOG_FILE"
		else
			if [[ $1 ]]; then
				#echo "[${LOG_LEVELS[$LEVEL]}] $1" | tee -a "$APP_LOG_FILE"
				echo "['echo $LOG_LEVELS | cut -d ' ' -f$((LEVEL + 1))'] $1" | tee -a "$APP_LOG_FILE"
			else
				#echo "[${LOG_LEVELS[$LEVEL]}] $(cat)" | tee -a "$APP_LOG_FILE"
				echo "['echo $LOG_LEVELS | cut -d ' ' -f$((LEVEL + 1))'] $(cat)" | tee -a "$APP_LOG_FILE"
			fi
		fi
	fi
}
